...
int main()
{
  ...
  if((file = fopen("960px-Lenna.bmp","rb")) == NULL)
    return -1;

  fread(&hdBMP,sizeof(hdBMP), 1, file);
  ...
  int pixelDataOffset = hdBMP.bfOffBits;
  ...
  // читаем в буфер
  fread(pixel_data, sizeof(unsigned char), num_bytes, file);
  ...
  // FILE *modified_image = fopen("modbmp.bmp", "w");
